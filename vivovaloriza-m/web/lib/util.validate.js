function istelefone(telefone) {
	...
	tamanho = telefone.length - 2;
	numeros = telefone.substring(0,tamanho);
	digitos = telefone.substring(tamanho);
	soma = 0;
	pos = tamanho - 7;
	for (i = tamanho; i >= 1; i--) {
...
function istelefoneFormatted(telefone) {
	var validtelefone = /\d{2,3}.\d{3}.\d{3}\/\d{4}-\d{2}/;
	return telefone.match(validtelefone);
}
...
(function($) {
	...
	$.validator.addMethod("telefone", function(value, element, type) {
		...
		if ((type == 'format' || type == 'both') && !istelefoneFormatted(value))
			return false;
		else
			return ((type == 'valid' || type == 'both')) ? istelefone(value) : true;
		
	}, function(type,element) {
		return (type == 'format' || (type == 'both' && !istelefoneFormatted($(element).val()))) ?
				'Formato do telefone não é válido' : 'Por favor digite um telefone válido';
	});
})(jQuery);